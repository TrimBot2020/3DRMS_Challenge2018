function [ image, dmap, visPoints ] = points2Image( points, imageSize, cam, tform, pointRadius, opacity, useMex )
...
[depths,idx] = sort(depths,'descend');
locs = locs(idx,:);
colours = colours(idx,:);
...
if(useMex)
    ...
    dmap = colourImage(locs, depths, round(disk), uint32(imageSize));
    dmap(dmap(:)==0) = inf;
else
           ...
           for ix = -pointRadius:pointRadius
               valid = (locs(i,1)+ix >= 0) & ...
                ...
                if(valid)
                    ...
                    dmap(locs(i,2)+iy+1, locs(i,1)+ix+1) = min(dmap(locs(i,2)+iy+1, locs(i,1)+ix+1), depths(i));
                end
           end
       end
    end
